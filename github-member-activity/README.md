...
Activity in a repository owned by the member's own GitHub account is excluded from every metric. Review events count distinct other-authored public pull requests with a human review interaction in the window. Discovery uses GitHub review contribution nodes and in-window PR conversation comments (`IssueComment` with a `pullRequest`). Contribution nodes are not timestamps: a candidate whose contribution time is outside the window is discarded only after the public PR's complete reviews connection confirms there is no submitted review by the member inside the window. Submitted reviews in any non-`PENDING` state count, including `COMMENTED`. Conversation comments, including `/lgtm`, also count. Multiple comments or reviews on the same PR collapse to one event at the earliest in-window interaction. Own pull requests are excluded. Bot-authored comments and reviews are ignored. Unsubmitted `PENDING` reviews and inline-only comments without a conversation comment or submitted review are not counted. In-window contribution candidates without an eligible review or conversation comment, incomplete pagination, identity conflicts, and unstable snapshots still fail validation.
